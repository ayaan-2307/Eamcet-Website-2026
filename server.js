...
app.get('/api/closing-ranks', (req, res) => {
    try {
        ...
        const dataPath = path.join(__dirname, 'data', 'closing-ranks-2026-phase1.json');
...
// Data directory
const DATA_DIR = path.join(__dirname, '2026 phase 1 data');
...
// Start server
app.listen(PORT, () => {
    console.log(`🚀 TS EAMCET 2026 Results Server running on http://localhost:${PORT}`);
    console.log(`📊 Data directory: ${DATA_DIR}`);
    
    // Check if data directory exists
    if (!fs.existsSync(DATA_DIR)) {
        console.warn('⚠️  Data directory not found. Please ensure the "2026 phase 1 data" folder exists.');
    } else {
        const colleges = fs.readdirSync(DATA_DIR, { withFileTypes: true })
